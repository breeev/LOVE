...
tkwidgets=['Tk','Button','Entry','Frame','Label','LabelFrame','OptionMenu','StringVar','Text','Scrollbar','Canvas']
exec('from tkinter import '+','.join(tkwidgets))
inventory={}
for wid in tkwidgets:inventory[wid]=[]
print(inventory)
...
def ex(e=None):
    global id
    for win in ['s','g','root']:
        try:exec(f'global {win};Close({win})')
        except:pass
    w.destroy()
def restart(e=None):
    ex()
...
def fontshowcase(e=None):
    ...
    canvas.create_window((4,4),Tk=frame,anchor="nw")
...
def settings(e=None):
    ...
    texts={
        'Customisation':[
            'Font family: ','Font size: ','FONT CHART','Background color: ','Foreground color: ','COLOR CHART','Panel side: ','Editor relief: ','Editor border size: '
        ],
        'Folders':['Text files: ','Midi files: ']
    }
    ...
    source=Entry(f,bg=bgC,fg=fgC,insertbackground=fgC,selectbackground=fgC,selectforeground=bgC)
    source.grid(column=1,row=0)
    t(source,Source)
    f10=Label(f,text="Export folder: ",bg=bgC,fg=fgC)
    f10.grid(column=0,row=1,sticky='E')
    export=Entry(f,bg=bgC,fg=fgC,insertbackground=fgC,selectbackground=fgC,selectforeground=bgC)
    export.grid(column=1,row=1)
    t(export,Export)
    butt=[Button(s,text="Apply",bg=bgC,fg=fgC,activebackground=fgC,activeforeground=bgC,command=Apply),        Button(s,text="Save",bg=bgC,fg=fgC,activebackground=fgC,activeforeground=bgC,command=Save),        Button(s,text="Close",bg=bgC,fg=fgC,activebackground=fgC,activeforeground=bgC,command=lambda:Close(s))]
    for i in range(0,3):butt[i].grid(column=i,row=2)
    s.mainloop()
...
def Apply():
    ...
    buttons=[*butt,*[btns[i] for i in btns],link]
    optionmenus=[o,q]
    texts=[text]
    family=ffamily.get()
    size=int(fsize.get())
...
Source='C:/Users/breva/Documents/synced/lov/userdata'# where to find text files
Export='C:/Users/breva/Documents/synced/lov/export'# where to put midi files"""
