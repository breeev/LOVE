...
def load(e=None):
    ...
    if saved or askokcancel('Confirm Load','Your document will be lost.'):
        opened=askopenfilename(defaultextension='mid',filetypes=(('Midi Files',['.mid','.midi'])))
        if not opened:return report('canceled loading')
        new()
        with open(opened,'r')as f:text.insert('1.0',f.read())
        report('loaded '+opened)
def save(e=None):
    global saved
    if not saved:
        ...
        p.write_text(data)
        saved=True
        report('document saved at '+p.absolute())
def play(e=None):
    report('playing')
    try:interpreter(text.get('1.0','end-1c'),Exe);report('playing with '+Exe)
    except Exception as e:showerror('Error',e);report(e)
def export(e=None):
    report('exporting')
    try:interpreter(text.get('1.0','end-1c'),export=Exp);report('file exported in '+Exp)
    except Exception as e:showerror('Error',str(e).title());report(e)

def colorshowcase(e=None):
    global inventory,g
    g=Window()
    report('opened color panel')
    g.bind('<Control-q>',ex)
    g.geometry('1164x800')
    i=0
    for u in colors:
...
def settings(e=None):
    ...
    except:s=Window();report('opened settings')
    s.bind('<Control-q>',ex)
...
def Apply():
    ...
    optionmenus=[o,q]
    texts=[text]
    report('getting values from settings')
    Family=family.get()
    Size=int(size.get())
    bgC=bgc.get()
    fgC=fgc.get()
