...
def report(s:str):
    ...
    delta=newtime-time
    idelta=int(delta)
    print(':'.join([str(i).zfill(2)for i in[idelta//360,idelta//60%60,idelta]]+[str(round(delta,3)).split('.')[1].zfill(3)]),s.title(),sep=' '*4)
class timer:
    def __init__(self,s):report(f'> {s}...')
    def __enter__(self):self.start=t()
    def __exit__(self,*args):report(f'< done [{t()-self.start}]')
with timer('importing libs'):
...
with timer('scanning preferences'):
...
def fill(w:Entry,t:str):
    """Modify the text of a Tkinter Entry object."""
    w.delete(0,'end')
    w.insert(0,t)
def ex(e=None,warn=True):
    global saved
    if saved or (not warn) or askokcancel('Confirm Exit','Your document will be lost.'):
        with timer('destroying other windows'):
            ignored=[]
            for win in ['s','g','root']:
                try:exec(win+'.destroy()');report('destroyed window '+win)
                except:ignored.append(win)
            w.destroy()
            rmtree('.cache',1)
        report('main window destroyed (ignoring '+','.join(ignored)+')')
    else:report('canceled exit')
def restart(e=None):
    global saved
    if saved or askokcancel('Confirm Restart','Your document will be lost.'):
        with timer('restarting'):
            ex(warn=False)# don't ask again for confirmation in case the doc wasn't saved
            from os import execl
            from sys import executable as e,argv
            execl(e,e,*argv)
    else:report('canceled restart')
def new(e=None):
    global saved,text
    if saved or askokcancel('Confirm New','Your document will be lost.'):
        with timer('new document'):
            text.delete('1.0','end-1c')
            saved=True
def load(e=None):
    global saved
    if saved or askokcancel('Confirm Load','Your document will be lost.'):
        opened=askopenfilename(defaultextension='.txt',filetypes=(['Text Files','.txt'],),initialdir=Src)
        if not opened:return report('canceled loading')
        with timer('loading file '+opened):
            new()
            with open(opened,'r')as f:text.insert('1.0',f.read())
def save(e=None):
    global saved
    if not saved:
...
def colorshowcase(e=None):
    global inventory,g
    with timer('opening color panel'):
        g=Window()
        g.bind('<Control-q>',ex)
        g.geometry('1164x800')
        i=0
...
def settings(e=None):
    ...
    try:s.focus_force();report('focusing on settings window')
    except:
        with timer('opening settings'):
            s=Window()
            s.bind('<Control-q>',ex)
            ...
            def SaveLastClickPos(event):
                ...
                report('settings window grabbed')
                lastClickX=event.x
                lastClickY=event.y
            def Dragging(event):
...
with timer('building main window'):
    chars={
        'exit':'\u2716',
        'restart':'\u2724',
        'new':'\u271A',
    ...
    def chsizeloop():
        ...
        except Exception as e:report(f'Background interpreter failed: {e}')
        finally:
